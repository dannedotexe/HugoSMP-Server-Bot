...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Zeigt deine Invite-Statistiken.')
];
...
async function registerCommands(guild) {
  ...
  try {
    ...
    console.error(`Fehler beim Registrieren der Commands für Guild ${guild.id}:`, error);
  }
}
...
function buildPanel() {
  ...
  const row = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder()
      .setCustomId('claim')
      .setLabel('Claim 1M') // <-- Hier geändert!
      .setEmoji('💰')
...
async function cacheInvites(guild) {
  try {
    ...
    console.error("Fehler beim Cachen der Invites:", err);
  }
}
...
client.on('interactionCreate', async interaction => {
  if (interaction.isChatInputCommand()) {
    if (interaction.commandName === 'setupinviterewards') {
      // Sendet das permanente Panel in den Kanal
      await interaction.channel.send(buildPanel());
      
      // Bestätigt den Befehl nur für den Admin sichtbar
      return interaction.reply({ content: '✅ Panel wurde gesendet.', ephemeral: true });
    }
  ...
  if (interaction.customId === 'check_inv') {
    ...
    return interaction.reply({ content: `📊 Du hast bisher **${count}/${REQUIRED_INVITES}** verifizierte Invites.`, ephemeral: true });
  }

  if (interaction.customId === 'claim') {
    const count = getInvites(interaction.user.id);
    if (count < REQUIRED_INVITES) {
      return interaction.reply({ content: `❌ Du brauchst noch ${REQUIRED_INVITES - count} weitere Invites.`, ephemeral: true });
    }

    resetInvites(interaction.user.id);
    const log = interaction.guild.channels.cache.get(REWARD_LOG_CHANNEL_ID);
    if (log) {
      await log.send(`💰 **${interaction.user.tag}** hat **${REWARD}** mit ${count} Invites beansprucht!`);
    }
    return interaction.reply({ content: `✅ Belohnung erfolgreich beansprucht!`, ephemeral: true });
  }
});
